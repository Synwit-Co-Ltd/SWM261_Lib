...
{	
	   ...
	   initStruct->DownCmpAIE || initStruct->UpCmpBIE || initStruct->DownCmpBIE)
	{
		...
		case((uint32_t)PWM2): NVIC_EnableIRQ(PWM2_IRQn);				break;
		case((uint32_t)PWM3): NVIC_EnableIRQ(GPIOA7_GPIOC7_PWM3_IRQn);	break;
		case((uint32_t)PWM4): NVIC_EnableIRQ(GPIOA8_GPIOC8_PWM4_IRQn);	break;
		}
	}
}
