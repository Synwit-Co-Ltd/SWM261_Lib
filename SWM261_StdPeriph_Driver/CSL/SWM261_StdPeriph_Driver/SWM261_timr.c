...
* 函数名称: TIMR_Init()
* 功能说明:	TIMR定时器/计数器初始化
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，有效值包括：TIMR0、TIMR1、TIMR2、TIMR3、BTIMR0、BTIMR1、BTIMR2、BTIMR3
...
{	
	...
	case ((uint32_t)BTIMR0):
		if(int_en) NVIC_EnableIRQ(GPI0A12_GPIOC12_BTIMR0_RTC_IRQn);
		break;
	
	case ((uint32_t)BTIMR1):
		if(int_en) NVIC_EnableIRQ(GPI0A13_GPIOC13_BTIMR1_WDT_IRQn);
		break;
	
	case ((uint32_t)BTIMR2):
		if(int_en) NVIC_EnableIRQ(GPI0A14_GPIOC14_BTIMR2_CORDIC_IRQn);
		break;
	
	case ((uint32_t)BTIMR3):
		if(int_en) NVIC_EnableIRQ(GPI0A15_GPIOC15_BTIMR3_XTALSTOP_IRQn);
		break;
	}
}
...
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，可取值包括：TIMR0、TIMR1、TIMR2、TIMR3、BTIMR0、BTIMR1、BTIMR2、BTIMR3
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
* 函数名称:	TIMR_Halt()
* 功能说明:	暂停定时器，计数值保持不变
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，可取值包括：TIMR0、TIMR1、TIMR2、TIMR3
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
