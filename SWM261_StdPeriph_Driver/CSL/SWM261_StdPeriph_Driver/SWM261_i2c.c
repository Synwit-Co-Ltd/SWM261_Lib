...
* 函数名称:	I2C_Init()
* 功能说明:	I2C初始化
* 输    入: I2C_TypeDef * I2Cx		指定要被设置的I2C，有效值包括I2C4、I2C5
...
void I2C_Init(I2C_TypeDef * I2Cx, I2C_InitStructure * initStruct)
{
	...
	{
	case ((uint32_t)I2C4):
		SYS->CLKEN0 |= (0x01 << SYS_CLKEN1_I2C4_Pos);
		break;
	
	case ((uint32_t)I2C5):
		SYS->CLKEN0 |= (0x01 << SYS_CLKEN1_I2C5_Pos);
		break;
	}
	...
	if(initStruct->Master == 1)
	{
		...
		case ((uint32_t)I2C5):		
			...
			{
				NVIC_DisableIRQ(GPI0A6_GPIOC6_I2C5_IRQn);
			}
			break;
		}
	}
	else
	{
		...
		if(initStruct->SlvSTADetIEn | initStruct->SlvSTODetIEn | initStruct->TXEmptyIEn | initStruct->RXNotEmptyIEn)
		{
			switch((uint32_t)I2Cx)
			{
			case ((uint32_t)I2C4): NVIC_EnableIRQ(I2C4_IRQn); 				break;
			case ((uint32_t)I2C5): NVIC_EnableIRQ(GPI0A6_GPIOC6_I2C5_IRQn); break;
			}
		}
