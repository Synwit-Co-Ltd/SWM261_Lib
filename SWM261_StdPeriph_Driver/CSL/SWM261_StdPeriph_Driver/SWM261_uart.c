...
void UART_Init(UART_TypeDef * UARTx, UART_InitStructure * initStruct)
{
	...
	case ((uint32_t)UART4):
		SYS->CLKEN0 |= (0x01 << SYS_CLKEN1_UART4_Pos);
		break;
	
	case ((uint32_t)UART5):
		SYS->CLKEN0 |= (0x01 << SYS_CLKEN1_UART5_Pos);
		break;
	}
	
	UART_Close(UARTx);	//一些关键寄存器只能在串口关闭时设置
	
	if((UARTx == UART4) || (UARTx == UART5)) clkdiv = 2;
	else									 clkdiv = 1;
	
	UARTx->BAUD &= ~(UART_BAUD_BAUD_Msk | UART_BAUD_FRAC_Msk);
	UARTx->BAUD |= (((SystemCoreClock/clkdiv/initStruct->Baudrate - 1) / 16) << UART_BAUD_BAUD_Pos) |
				   (((SystemCoreClock/clkdiv/initStruct->Baudrate - 1) % 16) << UART_BAUD_FRAC_Pos);
	...
	if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn)
	{
		...
		case ((uint32_t)UART2):	NVIC_EnableIRQ(GPIOA0_GPIOC0_UART2_IRQn);	break;
		case ((uint32_t)UART3):	NVIC_EnableIRQ(GPIOA1_GPIOC1_UART3_IRQn);	break;
		case ((uint32_t)UART4):	NVIC_EnableIRQ(GPI0A2_GPIOC2_UART4_IRQn);	break;
		case ((uint32_t)UART5):	NVIC_EnableIRQ(GPIOA3_GPIOC3_UART5_IRQn);	break;
		}
	}
}
...
* 函数名称:	UART_WriteByte()
* 功能说明:	发送一个字节数据
* 输    入: UART_TypeDef * UARTx	指定要被设置的UART串口，可取值包括UART0、UART1、UART2、UART3、UART4、UART5
*			uint8_t data			要发送的字节			
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
void UART_SetBaudrate(UART_TypeDef * UARTx, uint32_t baudrate)
{
	...
	UARTx->BAUD &= ~(UART_BAUD_BAUD_Msk | UART_BAUD_FRAC_Msk);
	UARTx->BAUD |= (((SystemCoreClock/clkdiv/baudrate - 1) / 16) << UART_BAUD_BAUD_Pos) |
				   (((SystemCoreClock/clkdiv/baudrate - 1) % 16) << UART_BAUD_FRAC_Pos);
}
...
uint32_t UART_GetBaudrate(UART_TypeDef * UARTx)
{
	...
	return SystemCoreClock/clkdiv/(((UARTx->BAUD & UART_BAUD_BAUD_Msk) >> UART_BAUD_BAUD_Pos) * 16 +
