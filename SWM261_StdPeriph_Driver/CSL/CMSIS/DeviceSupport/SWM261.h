...
typedef struct {
	...
	__IO uint32_t CR2;						//只存在于 ADC1 中
} ADC_TypeDef;


#define ADC_GO_ADC0SEQ0_Pos			0		//ADC0序列0转换启动位
...
#define ADC_GO_ADC1SEQ0_Pos			16		//ADC1序列0转换启动位
...
#define ADC_GO_RESET_Pos			28		//模拟模块复位
#define ADC_GO_RESET_Msk			(0x01 << ADC_GO_RESET_Pos)
#define ADC_GO_SIGN_Pos				30		//转换结果格式：0 无符号数   1 有符号数
#define ADC_GO_SIGN_Msk				(0x01 << ADC_GO_SIGN_Pos)
#define ADC_GO_SYNC_Pos				31		//0 ADC0和ADC1独立   1 ADC1与ADC0同步转换
...
#define ADC_CR_DIFF_Pos				3		//输入模式选择：0 单端模式   1 差分模式
#define ADC_CR_DIFF_Msk				(0x01 << ADC_CR_DIFF_Pos)
#define ADC_CR_STDLY_Pos			4		//Start Delay，SOC 信号后间隔 STDLY+1 个时钟周期开始采样
#define ADC_CR_STDLY_Msk			(0x1F << ADC_CR_STDLY_Pos)
#define ADC_CR_BITS_Pos				9		//转换结果位宽：0 12bit   1 10bit   2 8bit   3 6bit
#define ADC_CR_BITS_Msk				(0x03 << ADC_CR_BITS_Pos)
#define ADC_CR_DMAEN_Pos			12		//三位，每位对应一个序列，同一时刻只能一个有一个序列使能DMA，0 能通过CPU读取DATA_FIFO   1 只能通过DMA读取DATA_FIFO
#define ADC_CR_DMAEN_Msk			(0x07 << ADC_CR_DMAEN_Pos)
#define ADC_CR_FFCLR_Pos			16		//FIFO Clear，三位，每位对应一个序列
...
#define ADC_SEQTRG_SEQ0_Pos			0		//序列0触发选择，0x01 CPU   0x02 TIMR2   0x03 TIMR3   0x04-07 ADC_TRIG0-3引脚   0x10-14 PWM0-PWM4
...
#define ADC_SEQSMP_SEQ0_Pos			0		//序列0采样时间，0/1/2/3 采样建立时间保持1/2/4/8/16/32/64/128个采样时钟周期
