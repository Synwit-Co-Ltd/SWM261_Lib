...
#define ADC_SUB_SIZE  250	//SWM200 ADC 单次最多转换256次，2000可以被250整除
#define ADC_SUB_N   (ADC_SIZE / ADC_SUB_SIZE)
...
//	PORT_Init(PORTM, PIN0,  PORTM_PIN0_ADC0_CH15,  0);		//PM.0  => ADC0.CH15	SWCLK
	...
	*/
	...
	ADC_SEQ_initStruct.conv_cnt = ADC_SUB_SIZE;
...
#elif defined(CHIP_SWM200)
	DMA_initStruct.Count = ADC_SUB_SIZE;
...
void GPIOB1_GPIOA9_DMA_Handler(void)
{
	...
	if(DMA_CH_INTStat(DMA_CH1))
	{
		...
		DMA->CH[DMA_CH1].DST = (uint32_t)&ADC_Result[250*n];
