...
/* 特别注意：SWM201 ADC 使用 Continue 模式必须遵守以下要求：
 * 1、ADC_initStruct.trig_src 的赋值必须是 0 ，不能是 ADC_TRIGGER_SW
 * 2、启动 ADC 时必须使用 ADC0->START = 1; 不能使用 ADC_Start(ADC0);
 * 3、Continue 模式不能和 TIMR 触发、PWM 触发混合使用，分时复用也不行
 *****************************************************************************/
